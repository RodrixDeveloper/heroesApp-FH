...
export class NewPageComponent implements OnInit {
  ...
  onDeleteHero() {
    if (!this.currentHero.id) throw Error('Heroe id es requerido');
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: this.heroForm.value,
    });

    //? otra alternativa de codigo para simplificar
    dialogRef
      .afterClosed()
      .pipe(
    ...
    //   if (!result) return;
    //   console.log('ELIMANR');
    //   this._heroesService.deleteHeroById(this.currentHero.id).subscribe(wasDeleted=>{
    //     if(wasDeleted){
    //       this.router.navigate(['/heroes/list']);
    //     }
    //   })
